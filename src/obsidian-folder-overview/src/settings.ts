...
export class SettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		const span = createSpan({
			text: t('this won t apply to alre...#5ecba7'),
			cls: '',
		});
...
export async function createOverviewSettings(
	...
	createOrReplaceSetting(contentEl, 'allow-drag-and-drop', changedSection, (settingEl) => {
		new Setting(settingEl)
			.setName(t('Allow drag and drop'))
			.setDesc(t('Choose if you want to allow drag and drop in the overview'))
			.addToggle((toggle) =>
				toggle
	...
	createOrReplaceSetting(contentEl, 'showTitle', changedSection, (settingEl) => {
		new Setting(settingEl)
			.setName(t('Show the title'))
			.setDesc(t('Choose if the title abov...#9fa3ad'))
			.addToggle((toggle) =>
				toggle
	...
	createOrReplaceSetting(contentEl, 'title-size', changedSection, (settingEl) => {
		...
		new Setting(settingEl)
			...
			.setDesc(t('The larger the number th...#ddbc21'))
			.addSlider((slider) =>
				slider
	...
	createOrReplaceSetting(contentEl, 'use-wikilinks', changedSection, (settingEl) => {
		new Setting(settingEl)
			...
			.setDesc(t('Choose if the links in t...#82897f'))
			.addToggle((toggle) =>
				toggle
	...
	createOrReplaceSetting(contentEl, 'hide-link-list', changedSection, (settingEl) => {
		const hideLinkListSetting = new Setting(settingEl)
			.setName(t('Hide link list'))
			.setDesc(t('Choose if only link list...#8d22a0'))
			.addToggle((toggle) =>
				toggle
	...
	createOrReplaceSetting(contentEl, 'include-types', changedSection, (settingEl) => {
		...
		setting.setName(t('Include types'));
	...
	createOrReplaceSetting(contentEl, 'file-tag', changedSection, (settingEl) => {
		new Setting(settingEl)
			.setName(t('Disable file tag'))
			.setDesc(t('Choose if the file tag should be shown after the file name'))
			.addToggle((toggle) => {
				toggle
	...
	createOrReplaceSetting(contentEl, 'file-depth', changedSection, (settingEl) => {
		...
		const MAX_DEPTH = 10;
		new Setting(settingEl)
			.setName(t('File depth'))
			.setDesc(t('File & folder = +1 depth'))
			.addSlider((slider) =>
				slider
	...
	createOrReplaceSetting(
		...
		(settingEl) => {
			new Setting(settingEl)
				.setName(t('Only show empty folders...#964e66'))
				.addToggle((toggle) => {
					toggle
	...
	createOrReplaceSetting(contentEl, 'collapse-all-by-default', changedSection, (settingEl) => {
		new Setting(settingEl)
			.setName(t('Collapse all in the tree by default'))
			.setDesc(t('Collapse every folder in...#e0cd7e'))
			.addToggle((toggle) => {
				toggle
